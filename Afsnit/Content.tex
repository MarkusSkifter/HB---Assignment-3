...
\paragraph{Calibration.} Preference and technology parameters $\eta, \gamma, \rho, r, \alpha, p_b, \beta_0, \beta_1$ are set to standard values from the literature; the wage scales are normalised to $w_1 = w_2 = 1$ in the baseline (relaxing this allows a pre-birth gender gap); the tax rate $\tau_t$, replacement rate $\phi$, and total leave $\bar{L}$ come from Danish legislation and \emph{Barselstatistik}.

\paragraph{Disciplining the asymmetry.} The two leave-depreciation rates $\delta_1, \delta_2$ generate the child penalty. We discipline them with two targeted moments from \citet{kleven2019children}: $\delta_2$ matches the roughly 20-percentage-point drop in mothers' relative wages after first birth, and $\delta_1$ ensures essentially no father penalty.
